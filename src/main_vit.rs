...
fn main() {
    ...
    let n_layers = 2;
    let num_classes = 10;
    let seq_len = 28; // 28 rows of the image
    let patch_size = 28; // each row has 28 pixels
    ...
    println!("  d_model={}, n_heads={}, d_ff={}", d_model, n_heads, d_ff);
    println!("  Total parameters: {}", total_params);

    // Training hyperparameters
    let epochs = 3;
    let train_samples = 5000; // subset for speed
